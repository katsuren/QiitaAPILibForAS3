package flexUnitTests.com.k2rn.qiita.api
{
    ...
    public class GetTagItemListTest
    {		
        [Before]
        ...
        {
            var api:GetTagItemList = new GetTagItemList(TestHelper.API_END_POINT);
            api.tagId = "ruby";
        ...
        public function testSendException():void
        {
            ...
            try {
                api.send();
                Assert.fail(
                    "tagId 未設定のままでも例外が発生していません。\n" +
                    "tagId 未設定の場合は例外が発生すべきです。");
            }
            catch (err:Error) {
                Assert.assertEquals("tagId 未設定のままリクエストすると例外が発生する",
                    err.errorID, ErrorID.MISSING_TAG_ID);
            }
            api.tagId = "ruby";
        ...
        {
            var api:GetTagItemList = new GetTagItemList("http://some.invalid.end.point/");
            api.tagId = "some_dummy_tag_id";
